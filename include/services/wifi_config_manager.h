...
private:
    char ssid[33];        ///< WiFi网络名称(SSID)，最大32字符
    char password[65];    ///< WiFi密码，最大64字符
public:
    /**
     * @brief 构造函数
     * @param configFilePath WiFi配置文件的完整路径
     */
    WifiConfigManager(const char* configFilePath);
    ...
    /**
     * @brief 获取WiFi SSID
     * @return const char* WiFi网络名称
     */
    const char* getSSID();

    /**
     * @brief 获取WiFi密码
     * @return const char* WiFi密码
     */
